...
class ConvectorAccessory extends BaseAccessory {
  ...
  _registerCharacteristics(dps: DPSState): void {
    ...
    let characteristicTemperatureDisplayUnits: any
    if (!this.device.context.noTemperatureUnit) {
      characteristicTemperatureDisplayUnits = service
    ...
    const characteristicRotationSpeed = service
    ...
    this.device.on('change', (changes: DPSState, state: DPSState) => {
      ...
      if (this.platform?.reportTelemetry) {
        ...
        const currentTemperature = Number(state[this.dpCurrentTemperature]) || 0
        const targetTemperature = Number(state[this.dpDesiredTemperature]) || 0
    ...
    if (this.platform?.registerControl) {
      ...
      this.platform.registerControl(deviceId, 'targetTemperature', (value: unknown) => {
        this.setState(this.dpDesiredTemperature, Number(value), () => {})
      })
    }
  }
  ...
  convertRotationSpeedFromTuyaToHomeKit(value: DPSValue): number {
    return ({ [this.cmdLow]: 1, [this.cmdHigh]: 100 } as Record<string, number>)[value as string]
  }
