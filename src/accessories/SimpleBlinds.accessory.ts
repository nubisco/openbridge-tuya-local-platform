...
class SimpleBlindsAccessory extends BaseAccessory {
  ...
  _registerCharacteristics(dps: DPSState): void {
    ...
    this.minPosition = endingDuration
      ? Math.round((endingDuration * -100) / (this.duration - endingDuration))
      : BLINDS_CLOSED
    ...
    this.device.on('change', (changes: DPSState) => {
      ...
      if (changes.hasOwnProperty(this.dpAction)) {
        switch (changes[this.dpAction]) {
          ...
          case this.cmdStop: {
            ...
            this.log.debug(
              '[TuyaAccessory] Blinds last change was ' +
                this.changeTime +
                '; ' +
                (Date.now() - (this.changeTime as number)) +
                'ms ago',
            )

            if (this.changeTime) {
              const disposition = (Date.now() - (this.changeTime as number)) / (10 * this.duration)
            ...
            characteristicPositionState.updateValue(Characteristic.PositionState.STOPPED)
            this.log.debug(
              '[TuyaAccessory] Blinds marked stopped at ' +
                adjustedPosition +
                '; assumed to be at ' +
                this.assumedPosition,
            )
      ...
      if (this.platform?.reportTelemetry) {
        const position = Math.max(0, Math.round(this.assumedPosition))
        this.platform.reportTelemetry(this.device.context.id, { active: position > 0 })
      }
    })
    ...
    if (this.platform?.registerControl) {
      ...
      this.platform.registerControl(deviceId, 'active', (value: unknown) => {
        const cmd = value ? this.cmdOpen : this.cmdClose
        this.setState(this.dpAction, cmd, () => {})
      })
    }
  }
  ...
  setTargetPosition(value: DPSValue, callback: OpenbridgeCallback): void {
    ...
    if (this.changeTime !== false) {
      this.log.debug(
        '[TuyaAccessory] Blinds ' +
          (this.assumedState === BLINDS_CLOSING ? 'closing' : 'opening') +
          ' had started ' +
          this.changeTime +
          '; ' +
          (Date.now() - (this.changeTime as number)) +
    ...
    const duration = Math.abs(this.assumedPosition - numValue) * this.duration * 10

    if (Math.abs(numValue - this.assumedPosition) < 1) {
      return this.setState(this.dpAction, this.cmdStop, callback)
    } else if (numValue > this.assumedPosition) {
      this.assumedState = BLINDS_OPENING
    ...
    if (numValue !== BLINDS_OPEN && numValue !== BLINDS_CLOSED) {
      this.log.debug('[TuyaAccessory] Blinds will stop in ' + duration + 'ms')
      this.log.debug(
        '[TuyaAccessory] Blinds assumed started ' +
          this.changeTime +
          '; ' +
          (Date.now() - (this.changeTime as number)) +
          'ms ago',
